...
// Select the configured serial port
  ...
  #define USARTx_IRQn USART2_LPUART2_IRQn
...
USARTx_IRQHandler(void)
{
    ...
    if (sr & (USART_ISR_RXNE_RXFNE | USART_ISR_ORE))
        serial_rx_byte(USARTx->RDR);
    if (sr & USART_ISR_TXE_TXFNF && USARTx->CR1 & USART_CR1_TXEIE_TXFNFIE) {
        uint8_t data;
...
{
    USARTx->CR1 = CR1_FLAGS | USART_CR1_TXEIE_TXFNFIE;
}
